...
export default function SubscriptionModal({ isOpen, onClose, apps }: SubscriptionModalProps) {
  ...
  return (
    <div id="subscription-modal" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-xs">
      <div className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-2xl transition-all duration-300 animate-in fade-in zoom-in-95">
        ...
        {/* Modal Content tabs */}
        <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100 max-h-[85vh] overflow-y-auto">
          
          {/* Email Subscription Setup */}
          <div className="p-6">
            <div className="flex items-center gap-2 text-slate-800 font-bold mb-4">
              <Mail className="h-5 w-10 flex-shrink-0" />
              <h3 className="text-md font-semibold text-slate-800">Email Notification Crew</h3>
            </div>

            {isSubscribed ? (
              <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-5 text-center transition">
                <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
                  <Bell className="h-6 w-6 animate-pulse" />
                </div>
                <h4 className="mt-3 font-semibold text-slate-800">You are on the list!</h4>
                <p className="mt-1 text-xs text-slate-500 leading-relaxed">
                  We have registered <strong className="text-slate-700">{email}</strong> to receive {frequency} developer notifications for {selectedApps.length === 0 ? "all hubs" : `${selectedApps.length} specified tools`}.
                </p>
                <button
                  id="reset-sub-btn"
                  onClick={() => {
                  ...
                  className="mt-4 text-xs font-semibold text-slate-800 underline hover:text-slate-950"
                >
              ...
              <form onSubmit={handleSubscribeEmail} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-800 placeholder-slate-400 focus:border-slate-550 focus:outline-none focus:ring-1 focus:ring-slate-500"
                  />
                </div>
                      ...
                      <button
                        ...
                        className={`flex items-center justify-between rounded-lg border px-2.5 py-1.5 text-left text-xs transition ${
                          selectedApps.includes(app.id)
                            ? "border-slate-800 bg-slate-50/80 text-slate-905 font-semibold"
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-605 mb-1 animate-none">SEND FREQUENCY</label>
          ...
          <div className="p-6 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 text-slate-800 font-bold mb-4">
                ...
                <h3 className="text-md font-semibold text-slate-800">RSS & Developer Feeds</h3>
              </div>
              ...
              <div className="space-y-3.5">
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3 flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="rounded-md bg-orange-100 p-1 text-orange-600">
                        <Rss className="h-3.5 w-3.5" />
                      </div>
                      <span className="text-xs font-bold text-slate-705">RSS / XML Format (Feed.xml)</span>
                    </div>
                    <button
                      id="copy-rss-btn"
                      onClick={() => handleCopy("rss", rssUrl)}
                      className="rounded-md bg-white border border-slate-200 p-1 text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition"
                      title="Copy URL"
                    >
                      {copiedFeed === "rss" ? <Check className="h-3.5 w-3.5 text-emerald-500" /> : <Copy className="h-3.5 w-3.5" />}
                    </button>
                  </div>
                  <code className="text-[10px] text-slate-500 select-all font-mono break-all bg-white rounded-lg p-1.5 border border-slate-200/50">
                    {rssUrl}
                  </code>
                </div>
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3 flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="rounded-md bg-cyan-100 p-1 text-cyan-700">
                        <Link2 className="h-3.5 w-3.5" />
                      </div>
                      <span className="text-xs font-bold text-slate-700">JSON Feed Format (Feed.json)</span>
                    </div>
                    <button
                      id="copy-json-btn"
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50/60 p-3 space-y-2">
                  ...
                  {webhookRegistered ? (
                    <div className="rounded-lg bg-emerald-50 border border-emerald-100 p-2 text-[11px] text-emerald-800 flex items-center justify-between">
                    ...
                    <div className="flex gap-1.5">
                      <input
                        ...
                        className="flex-1 text-[11px] rounded-lg border border-slate-250 bg-white px-2 py-1 focus:outline-none focus:border-slate-500"
                      />
                      <button
